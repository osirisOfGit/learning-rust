use std::{ops::{Add, Div, Mul}, path::Path, thread::sleep, time::{self, Duration}};
...
use bevy::{
    ...
    log::error,
    math::{Rect, Vec3},
    prelude::{Camera2dBundle, Commands, DefaultPlugins, PluginGroup, Query, Res, WindowPlugin, default},
...
fn setup(mut commands: Commands, windows: Query<&Window>, asset_server: Res<AssetServer>) {
    ...
    match tiled_loader.load_tmx_map("assets/checkers_board.tmx") {
        Ok(map) => {
            ...
            let top_left_coord = (
                0. - resolution.width().div(2.),
                0. - resolution.height().div(2.),
            );
                            ...
                            commands.spawn(SpriteBundle {
                                sprite: Sprite {
                                    rect: Some(Rect::new(
                                        0.,
                                        0.,
                                        layer_tile.get_tileset().tile_width as f32,
